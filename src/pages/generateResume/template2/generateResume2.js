              ...
              <div className="gr_resume_container">
                      ...
                      {resumeInfo?.education?.map((ed) => {
                        return (
                          <li key={ed.id} className="education-item">
                            <h5>{extractYearFromISODate(ed?.startYear)} - {extractYearFromISODate(ed?.endYear)}</h5>
                  ...
                  <div className="language">
                    ...
                    <ul>
                      {resumeInfo?.skills?.map((s) => {
                        const skill = s?.ratting; // Replace with your skill rating value

                        // Convert rating to a percentage
                        const percentage = (skill / 10) * 100;
                        return (
                          <li key={s?.id}>
                            <span className="text">{s?.skill}</span>
                ...
                <div className="right_side">
                          ...
                          <div className="box">
                            <div className="year_company">
                              <h5>
                                ...
                                -{" "}</h5>
                                <h5> {extractYearFromISODate(ex?.endYear) === null
                                  ? "Present"
                                  : extractYearFromISODate(ex?.endYear)}
                                  </h5>
                  ...
                  {resumeInfo?.personalProject?.isPersonalProject === false ? (
                    <div className="about">
                      <h2 className="title2">Personal Project</h2>
                      {resumeInfo?.personalProject?.projects?.map((ex) => {
                        return (
                          <div className="box">
                            <div className="year_company">
                              <h5>
                                {extractYearFromISODate(ex?.startDate) || 2019}{" "}
                                -{" "}</h5>
                                <h5> {extractYearFromISODate(ex?.endDate) === null
                                  ? "Present"
                                  : extractYearFromISODate(ex?.endDate)}
                              </h5>
                              <h5>{ex?.projectTitle}</h5>
                            </div>
                            <div className="text">
                              <h4>{ex?.role}</h4>
                              <p>{ex?.projectDetail}</p>
                            </div>
                          </div>
                        );
                      })}
                  ...
                  <div className="about">
                    ...
                    {resumeInfo?.education?.map((ex) => {
                      return (
                        <div className="box">
                          <div className="year_company">
                            <h5>
                              {extractYearFromISODate(ex?.startYear) || 2019} -{" "}</h5>
                              <h5>{extractYearFromISODate(ex?.endYear) === null
                                ? "Present"
                                : extractYearFromISODate(ex?.endYear)}
                            </h5>
                            <h5>{ex?.collegeName}</h5>
                          </div>
                          <div className="text">
                            <h4>{ex?.courseName}</h4>
                            <p>{`Post Grduation Diploma in Advance Computing`}</p>
                          </div>
                        </div>
                      );
                    })}
